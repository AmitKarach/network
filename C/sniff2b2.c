...
/* Ethernet header */ // Took from UNI // Adjusted to work and  sniff tcp packects with specific ports
struct ethheader
{
...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    if (ntohs(eth->ether_type) == 0x0800)
    {
        ...
        if (ip->iph_protocol == IPPROTO_TCP) // Only Attend to Tcp 
        {
...
int main() // Took From UNI
{
    ...
    char filter_exp[] = "proto TCP and dst portrange 10-100"; // Filter Tcp Packets
    bpf_u_int32 net;

    handle = pcap_open_live("br-c64184acb1a0", BUFSIZ, 0, 1000, errbuf); // Changed According to interface
